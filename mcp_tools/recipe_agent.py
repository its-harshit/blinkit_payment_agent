...
class RecipeAgent:
    ...
    async def _ensure_blinkit(self):
        if self.blinkit_client is None:
            ...
            self.blinkit_client = McpClient("blinkit-recipe-agent", BLINKIT_CMD, cwd=str(MCP_TOOLS_DIR))
    ...
    async def _ensure_payment(self):
        if self.payment_client is None:
            ...
            self.payment_client = McpClient("payment-recipe-agent", PAYMENT_CMD, cwd=str(MCP_TOOLS_DIR))
