...
SHOPPING_INSTRUCTIONS = (
    ...
    "- list_blinkit_discounts_tool(amount, order_id?): List Blinkit promo codes eligible for this cart total. Use at checkout before create_payment.\n"
    "- apply_blinkit_discount_tool(code, amount, order_id?): Apply a Blinkit code; returns valid, finalAmount, message. Use finalAmount in create_payment if valid.\n"
    "- create_payment(amount, order_id): Create payment intent (amount in INR, order_id optional). Use discounted finalAmount if user applied a Blinkit code.\n"
    ...
    "- User: 'Checkout and pay' → Use view_cart() to get total. Optionally call list_blinkit_discounts_tool(amount=total); if user picks a code, call apply_blinkit_discount_tool and use finalAmount. Then create_payment(amount=total or finalAmount), then check_payment_status(payment_id=...) to complete payment and clear cart.\n"
    "\n"
    ...
    "7. If user wants to proceed to checkout: (a) call view_cart to get the cart total and then ask the user if he wants you to check for discounts and offers."
    "8. If user confirms, calllist_blinkit_discounts_tool(amount=cart_total) and show eligible Blinkit offers; if user selects a code, call apply_blinkit_discount_tool and use the returned finalAmount; (c) call create_payment with that amount (or full total if no discount); (d) then call check_payment_status with the payment_id from create_payment to complete the transaction and clear the cart.\n"
